...
startPercentageString = dbutils.widgets.get("start_percentage")
endPercentageString = dbutils.widgets.get("end_percentage")
print(f"start: {startPercentageString}")
print(f"end: {endPercentageString}")

startPercentage = float(startPercentageString)
endPercentage = float(endPercentageString)
...
# Define this here so we can flag the areas that are being used
statsMap = {}
...
with gzip.open(weights_file_local, mode="rt") as bedfile:
    ...
    for row in bedreader:
        ...
        if weight != 0:
            ...
            # Keep going until we hit the area to work in
            if cumulativePercentage < startPercentage:
                continue
            
            # If we're done, we're done
            if cumulativePercentage > endPercentage:
                print(f"Ending processing because {cumulativePercentage} has crossed the threshold of {endPercentage}")
                break
            
            statsKey = location - location % statsBlockSize
            # Make sure there's an entry here so we know which stats to keep later
            statsMap[statsKey] = {}
...
statsCount = 0
statsSkip = 1000
# This is now defined above so we can see which ones to keep
# statsMap = {}
with gzip.open(stats_local, mode="rt") as statsFile:
    ...
    for row in statsReader:
        ...
        # for the entries we want to keep, 
        if locationBin not in statsMap:
            continue
...
with gzip.open(spike_file_local, mode="rt") as bedfile:
    ...
    for row in bedreader:
        ...
        # Does this overlap with any of the blocks that we kept stats for?  That's a good proxy for whether we should keep this, just blockier
        keyToCheck = bin - bin % statsBlockSize
        if keyToCheck not in statsMap:
            continue
...
nearestKey = closest(segmentDictionary, random.uniform(startPercentage,endPercentage))
...
alt_allele_w_fake = "alt_allele_w_fake_v3" 
