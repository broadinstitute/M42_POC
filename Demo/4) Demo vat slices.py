...
raw_table_name = f"{schema}vat_slices_v0"
clustered_raw_table_name = f"{schema}vat_slices_clustered_v0"
...
print(f"writing to {raw_table_name}")
spark.sql(f"DROP TABLE IF EXISTS {raw_table_name}")

# reorganize columns so that the tables we want to cluster are first
...
print(f"writing to {clustered_raw_table_name}")
...
# MAGIC select gene_symbol, count(*) from cmotters_m42.demo.vat_slices_v0 group by gene_symbol
...
# MAGIC   FROM cmotters_m42.demo.vat_slices_v0 
# MAGIC   WHERE 
...
# MAGIC   FROM variants, cmotters_m42.demo.alt_allele as aa
...
# MAGIC WITH variants as (
# MAGIC   SELECT DISTINCT vid, contig, position, ref_allele, alt_allele, gvs_all_sc
# MAGIC   FROM cmotters_m42.demo.vat_slices_clustered_v0 
# MAGIC   WHERE 
# MAGIC   is_canonical_transcript = TRUE
# MAGIC   AND concat_ws('.', clinvar_classification) LIKE '%athogenic%'
# MAGIC   AND concat_ws('.', clinvar_classification) NOT LIKE '%benign%'
